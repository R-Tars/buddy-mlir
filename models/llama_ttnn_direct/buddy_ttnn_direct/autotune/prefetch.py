...
def build_prefetch_phase_report(
    *,
    official_ab: Mapping[str, Any],
    buddy_region_ab: Mapping[str, Any],
    full_decode_assessments: Sequence[Mapping[str, Any]],
    execution_contract: Mapping[str, Any],
) -> dict[str, Any]:
    assessments = {
        str(item.get("scope")): dict(item)
        for item in full_decode_assessments
    }
    missing = set(PREFETCH_REQUIRED_FULL_DECODE_SCOPES) - set(assessments)
    if missing:
        raise PrefetchAuditError(
            "missing full-decode prefetch assessments: "
            + ", ".join(sorted(missing))
        )
    promoted = [
        item for item in assessments.values() if bool(item.get("promoted"))
    ]
    winner = None
    if promoted:
        winner = max(
            promoted,
            key=lambda item: float(item.get("median_gain") or -math.inf),
        )["scope"]

    official_completed = bool(official_ab.get("completed"))
    region_completed = buddy_region_ab.get("status") == "passed"
    full_completed = all(
        item.get("status")
        in {"measured", "static_rejected", "hardware_rejected"}
        for item in assessments.values()
    )
    phase_completed = official_completed and region_completed and full_completed
    return {
        "schema_version": PREFETCH_SCHEMA_VERSION,
        "stage": "existing-api-blackhole-prefetch",
        "status": "passed" if phase_completed else "failed",
        "phase_completed": phase_completed,
        "execution_contract": dict(execution_contract),
        "official_current_source_ab": dict(official_ab),
        "buddy_mlp_region_ab": dict(buddy_region_ab),
        "full_decode_candidates": assessments,
        "promotion": {
            "selected_scope": winner,
            "default_enabled": winner is not None,
            "reason": (
                "full-model promotion gate passed"
                if winner is not None
                else "no legal full-model candidate passed the promotion gate"
            ),
        },
        "local_region_opportunity_observed": bool(
            buddy_region_ab.get("enter_full_model")
        ),
