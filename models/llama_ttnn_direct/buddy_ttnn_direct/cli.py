...
def build_parser() -> argparse.ArgumentParser:
    ...
    legacy_parsers: dict[str, argparse.ArgumentParser] = {}

    class ProductCommandRegistry:
        def add_parser(
            self,
            name: str,
            *args: object,
            **kwargs: object,
            ...
            if name in PRODUCT_COMMANDS:
                return product_subparsers.add_parser(name, *args, **kwargs)
            legacy = argparse.ArgumentParser(
                prog=f"buddy-ttnn-direct {name}",
                description=kwargs.get("description"),
            )
            legacy_parsers[name] = legacy
            return legacy

    subparsers = ProductCommandRegistry()
    ...
    parser._legacy_command_parsers = legacy_parsers
    return parser
...
def main(argv: list[str] | None = None) -> int:
    ...
    legacy_parsers = getattr(parser, "_legacy_command_parsers", {})
    if argv is not None and raw_argv and raw_argv[0] in legacy_parsers:
        args = legacy_parsers[raw_argv[0]].parse_args(raw_argv[1:])
        args.command = raw_argv[0]
    else:
