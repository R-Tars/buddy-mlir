...
linear.qkv_packed nlp_create_qkv_heads_decode rotary_embedding_decode
paged_update_cache paged_scaled_dot_product_attention_decode
nlp_concat_heads_decode linear.o_proj
""".split()


class AttentionDecodeTemplateTest(unittest.TestCase):
    def test_official_attention_decode_op_sequence(self) -> None:
        self.assertEqual(official_paged_attention_decode_op_sequence(), EXPECTED_ATTENTION_OPS)
    ...
    def test_plan_expansion_uses_attention_template_ops(self) -> None:
        self.assertEqual(expand_layer_templates(["official_paged_attention_decode"]), EXPECTED_ATTENTION_OPS)
