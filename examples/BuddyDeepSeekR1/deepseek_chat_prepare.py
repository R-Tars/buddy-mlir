...
def _prepare_phase(
    ...
    if phase == "prefill":
        ...
        with torch.no_grad():
            ...
            # Transformers 5.5: model.forward no longer accepts cache_position
            # so we advance each layer's `cumulative_length` buffer instead. The
            # patched `update` reads that buffer, keeping it visible to Dynamo
            # as a graph-level placeholder.
    ...
    for slot, ph_name in enumerate(sg_input_names):
        ...
        if not is_input:
            ...
            if dt == "int64" and shp == [1, L]:
                ...
                role = "cache_position"
            elif dt == "bfloat16" and shp == [1, 2, L, 128]:
                if kv_seen % 2 == 0:
